...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		maze.panel.col.setColor(Color.BLACK);
		maze.panel.fillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		maze.panel.col.setColor(Color.DKGRAY);
		maze.panel.fillRect(0, view_height/2, view_width, view_height/2);
		// set color to white and draw what ever can be seen from the current position
		maze.panel.col.setColor(Color.WHITE);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		maze.panel.setColor(seg.col.getColor());
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			maze.panel.fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
