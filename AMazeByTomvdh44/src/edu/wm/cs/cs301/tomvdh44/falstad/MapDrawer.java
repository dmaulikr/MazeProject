...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(int state, int px, int py,
	...
	public void draw_map(int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		maze.panel.setColor(Color.WHITE);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				maze.panel.setColor(seencells.hasWallOnTop(x, y) ? Color.WHITE : Color.GRAY);
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					maze.panel.drawLine(nx1, ny1, nx2, ny1);
				...
				maze.panel.setColor(seencells.hasWallOnLeft(x, y) ? Color.WHITE : Color.GRAY);
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					maze.panel.drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
	...
	public void draw_currentlocation(int view_dx, int view_dy) {
		...
		maze.panel.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		maze.panel.drawLine(ctrx, ctry, aptx, apty);
		maze.panel.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		maze.panel.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(int offx, int offy, int px, int py) {
		...
		maze.panel.setColor(Color.YELLOW);
		...
		while (d > 1) {
			...
			if (4 == n)
			{
				Log.v("MapDrawer", "ERROR: draw_solution cannot identify direction towards solution!");
				return ;
			}
			...
			// do the graphics
			maze.panel.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
